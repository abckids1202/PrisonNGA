...
| `/api/control/resources` | Staff resource permission | Staff profile facility | Expected resource/waiting versions; idempotent reassignment and kiosk credential lifecycle replay; active-assignment guard for maintenance/offline status | Resource status/reassignment/credential audit | Real device enrollment, heartbeat and recovery procedure |
