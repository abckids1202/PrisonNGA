...
| `/api/auth/visitor/verify` | One-time visitor OTP | User account scope | Challenge expiry, attempt cap, one-time consumption | Visitor login and failed-challenge security events | Production delivery, suspicious-login response and cookie/domain staging |
...
| `/api/control/resources` | Staff resource permission | Staff profile facility | Expected resource/waiting versions | Resource status/reassignment audit | Real device enrollment, heartbeat and recovery procedure |
| `/api/control/verification*` | Staff verification permission | Case/document facility | Version/idempotency on decisions | Review decision history and audit/outbox | Institutional review policy and protected evidence staging |
