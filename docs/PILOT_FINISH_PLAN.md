...
- The latest automated baseline is 301 server tests and 14 browser tests. Development E2E covers signed payment settlement, duplicate webhook delivery, and the persisted verification-to-payment-to-appointment approval handoff through a local-only provider adapter; this proves the boundary, not readiness for a real payment provider.
