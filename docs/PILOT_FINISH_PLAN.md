...
- The latest automated baseline is 313 server tests and 16 browser tests. Development E2E covers signed payment settlement, duplicate webhook delivery, the persisted verification-to-payment-to-appointment approval handoff through a local-only provider adapter, and the management-to-operations resource handoff; this proves the boundary, not readiness for real external providers.
