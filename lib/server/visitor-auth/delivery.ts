...
export async function deliverVisitorChallenge(input: { challengeId: string; destination: string; code: string; expiresAt: string }): Promise<void> {
  ...
  const payload = JSON.stringify({ type: "VISITOR_AUTH_CODE", channel: "EMAIL", challengeId: input.challengeId, destination: input.destination, code: input.code, expiresAt: input.expiresAt });
