export async function purgeExpiredAuthArtifacts(db: Pick<D1Database, "prepare" | "batch">): Promise<void> {
  await db.batch([
    db.prepare("DELETE FROM auth_challenge_delivery_attempts WHERE challenge_id IN (SELECT id FROM auth_challenges WHERE expires_at <= CURRENT_TIMESTAMP OR (consumed_at IS NOT NULL AND consumed_at <= datetime('now', '-1 day')) )"),
