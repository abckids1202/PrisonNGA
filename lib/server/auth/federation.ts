...
export type StaffFederationConfig = { provider: "none" | "oidc" | "saml"; configured: boolean; missing: string[]; issuer?: string; entityId?: string; metadataUrl?: string };
...
export async function getStaffFederationConfig(): Promise<StaffFederationConfig> {
  ...
  if (provider === "oidc") {
    ...
    const missing = [!issuer ? "STAFF_OIDC_ISSUER" : "", !clientId ? "STAFF_OIDC_CLIENT_ID" : ""].filter(Boolean);
    ...
    return { provider, configured: missing.length === 0, missing, issuer: issuer || undefined };
  }
