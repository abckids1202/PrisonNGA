...
type VisitorAppointmentRecord = { id: string; facility_id: string; facility_name: string; prisoner_id: string; status: string; requested_start: string; requested_end: string; timezone: string; version: number; prisoner_name: string; appointment_type: string; created_at?: string; updated_at?: string };
  ...
  return (
    <div className="sv4-page">
      ...
      {nextVisit ? <section className="sv4-next-visit-card" aria-label={`NEXT VISIT with ${nextVisit.prisoner_name}`} onClick={onOpenVisit} role="button" tabIndex={0} onKeyDown={(event) => event.key === "Enter" && onOpenVisit()}>
        <div className="sv4-next-visit-main"><div className="sv4-card-overline"><span>Your next visit</span><VisitorStatus>APPROVED</VisitorStatus></div><h2>Visit scheduled</h2><div className="sv4-person-row"><VisitorAvatar initials={nextVisit.prisoner_name.split(" ").map((part) => part[0]).join("").slice(0, 2)} /><div><strong>{nextVisit.prisoner_name}</strong><span>{nextVisit.appointment_type} visit · {nextVisit.facility_name}</span></div></div></div>
