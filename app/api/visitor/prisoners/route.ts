...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await d1.prepare(`SELECT p.id, p.facility_id, p.prisoner_number, p.display_name, p.status, p.visitation_status, vr.id AS relationship_id, COALESCE(vr.status, 'NOT_CONNECTED') AS relationship_status
      FROM prisoners p LEFT JOIN visitor_relationships vr ON vr.prisoner_id = p.id AND vr.visitor_user_id = ?
      WHERE p.status = 'ACTIVE' AND p.visitation_status = 'APPROVED' ${facilityId ? "AND p.facility_id = ?" : ""} ORDER BY p.display_name ASC LIMIT 200`).bind(...(facilityId ? [visitor.userId, facilityId] : [visitor.userId])).all();
