...
export async function POST(request: Request) {
  ...
  try {
    ...
    const d1 = await getD1();
    const current = await d1.prepare(`SELECT a.id, a.status AS appointment_status, a.version AS appointment_version, f.current_state,
    ...
    const results = await d1.batch([
      ...
      d1.prepare(`INSERT INTO outbox_events (id, event_type, aggregate_type, aggregate_id, facility_id, payload, correlation_id, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(crypto.randomUUID(), `WAITING_ROOM_${command.toUpperCase()}`, "appointment", body.appointmentId, authorization.facilityId, JSON.stringify({ appointmentId: body.appointmentId, command, state: nextState, kioskId: body.kioskId || null }), correlationId, now),
    ]);
    if (!results[1]?.meta.changes) throw new SecurityError("STALE_APPOINTMENT_STATE", 409);
    return securityResponse({ appointmentId: body.appointmentId, state: nextState, version: nextVersion, correlationId }, 200, context.requestId);
  } catch (error) {
    return securityErrorResponse(error, context.requestId);
  }
}
