...
export async function POST(request: Request) {
  ...
  try {
    ...
    const operationallyEligible = prisonerEligible && facilityEligible;
    ...
    const nextState = command === "admit_visitor" ? "VISITOR_WAITING" : command === "mark_late" ? "LATE" : command === "start_visit" ? "LIVE" : command === "contact_visitor" ? currentState : command === "cancel_visit" ? "CANCELLED" : (command === "run_preflight" || command === "retry_device") && !operationallyEligible ? "STAFF_REVIEW" : "READY_TO_START";
    ...
    const restrictionState = command === "run_preflight" || command === "retry_device" || command === "start_visit"
      ? operationallyEligible ? "pass" : "failed"
      : String(current.restriction_state || "pending");
    ...
    const statements = [
      ...
      d1.prepare(`INSERT INTO waiting_room_sessions (appointment_id, facility_id, state, visitor_presence, prisoner_presence, identity_state, camera_state, microphone_state, network_state, room_state, kiosk_state, restriction_state, assigned_room_id, assigned_kiosk_id, staff_notes, version, last_checked_at, created_at, updated_at)
        ...
        .bind(body.appointmentId, authorization.facilityId, nextState, visitorPresence, prisonerPresence, checkState, checkState, checkState, checkState, "pass", restrictionState, body.roomId?.trim() || String(current.assigned_room_id || "") || null, command === "reassign_kiosk" ? body.kioskId?.trim() : String(current.assigned_kiosk_id || "") || null, body.staffNotes?.trim().slice(0, 500) || null, nextVersion, now, now, now),
