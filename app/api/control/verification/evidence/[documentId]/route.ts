...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    await d1.prepare(`INSERT INTO audit_events
      ...
      VALUES (?, ?, ?, ?, 'VERIFICATION_EVIDENCE_ACCESSED', 'evidence_document', ?, 'Authorized verification reviewer accessed submitted evidence.', NULL, NULL, ?, ?, ?)`)
      .bind(crypto.randomUUID(), authorization.userId, authorization.roles[0] || "Verification Officer", authorization.facilityId, evidence.id, correlationId, requestContext.requestId, new Date().toISOString())
      .run();
