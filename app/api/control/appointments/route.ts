...
export async function POST(request: Request) {
  ...
  try {
    ...
    const current = await d1.prepare(`SELECT a.id, a.facility_id, a.visitor_user_id, a.prisoner_id, a.status, a.version, a.requested_start, a.requested_end, a.timezone AS appointment_timezone, a.policy_version AS appointment_policy_version, a.duration_minutes, p.status AS prisoner_status, p.visitation_status, f.current_state AS facility_state, f.timezone AS facility_timezone, vp.version AS policy_version, vp.min_duration_minutes, vp.max_duration_minutes, vp.min_advance_minutes, vp.max_advance_days, vp.daily_start_time, vp.daily_end_time, ca.id AS credit_account_id, ca.available_credits,
      ...
      ,CASE WHEN EXISTS (SELECT 1 FROM facility_closures fc WHERE fc.facility_id = a.facility_id AND fc.status = 'ACTIVE' AND fc.starts_at < a.requested_end AND fc.ends_at > a.requested_start) THEN 1 ELSE 0 END AS closure_conflict
      FROM appointments a INNER JOIN prisoners p ON p.id = a.prisoner_id AND p.facility_id = a.facility_id INNER JOIN facilities f ON f.id = a.facility_id LEFT JOIN visit_policies vp ON vp.facility_id = f.id LEFT JOIN credit_accounts ca ON ca.user_id = a.visitor_user_id AND ca.facility_id = a.facility_id WHERE a.id = ? AND a.facility_id = ?`).bind(appointmentId, authorization.facilityId).first<{ id: string; facility_id: string; visitor_user_id: string; prisoner_id: string; status: string; version: number; requested_start: string; requested_end: string; appointment_timezone: string | null; appointment_policy_version: number | null; duration_minutes: number | null; prisoner_status: string; visitation_status: string; facility_state: string; facility_timezone: string | null; policy_version: number | null; min_duration_minutes: number | null; max_duration_minutes: number | null; min_advance_minutes: number | null; max_advance_days: number | null; daily_start_time: string | null; daily_end_time: string | null; credit_account_id: string | null; available_credits: number | null; active_credit_reservation: number; relationship_approved: number; closure_conflict: number }>();
    ...
    if (command === "approve" && current.closure_conflict) throw new SecurityError("FACILITY_CLOSED_FOR_REQUESTED_WINDOW", 409);
    ...
    if (!results[0]?.meta.changes) {
      const latest = await d1.prepare(`SELECT a.status, a.version, a.requested_start, a.requested_end, p.status AS prisoner_status, p.visitation_status, f.current_state AS facility_state, f.timezone AS facility_timezone, vp.version AS policy_version, vp.min_duration_minutes, vp.max_duration_minutes, vp.min_advance_minutes, vp.max_advance_days, vp.daily_start_time, vp.daily_end_time, ca.available_credits, ca.reserved_credits,
        ...
        FROM appointments a INNER JOIN prisoners p ON p.id = a.prisoner_id AND p.facility_id = a.facility_id INNER JOIN facilities f ON f.id = a.facility_id LEFT JOIN visit_policies vp ON vp.facility_id = f.id LEFT JOIN credit_accounts ca ON ca.user_id = a.visitor_user_id AND ca.facility_id = a.facility_id
