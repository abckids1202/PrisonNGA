...
function Button({ children, variant = "secondary", onClick, disabled = false, type = "button" }: { children: ReactNode; variant?: "primary" | "secondary" | "quiet" | "danger"; onClick?: () => void; disabled?: boolean; type?: "button" | "submit" | "reset" }) {
...
function WaitingRoomPage({ facilityState, onNotify }: { facilityState: string; onNotify: (message: string, tone?: Notice["tone"]) => void }) {
  ...
  const [query, setQuery] = useState("");
  const [windowFilter, setWindowFilter] = useState<"next_2_hours" | "today" | "all_approved">("next_2_hours");
  ...
  useEffect(() => {
    ...
    run();
    const timer = window.setInterval(() => { if (active) { setWindowNow(Date.now()); run(); } }, 15000);
    return () => { active = false; window.clearInterval(timer); };
  }, []);
  ...
  const windowRecords = baseRecords.filter((record) => {
    const start = record.requestedStart ? Date.parse(record.requestedStart) : Number.NaN;
    const end = record.requestedEnd ? Date.parse(record.requestedEnd) : Number.NaN;
    if (!Number.isFinite(start) || !Number.isFinite(end)) return windowFilter === "all_approved";
    if (windowFilter === "all_approved") return true;
    const now = windowNow;
    if (windowFilter === "next_2_hours") return end >= now && start <= now + 2 * 60 * 60 * 1000;
    const day = new Intl.DateTimeFormat("en-CA", { timeZone: record.timezone || "Asia/Jakarta", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(now));
    const visitDay = new Intl.DateTimeFormat("en-CA", { timeZone: record.timezone || "Asia/Jakarta", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(start));
    return day === visitDay && end >= now;
  });
  const records = windowRecords.filter((record) => `${record.visitor} ${record.prisoner} ${record.id} ${record.room} ${record.kiosk}`.toLowerCase().includes(query.toLowerCase())).filter((record) => !onlyAttention || ["TECHNICAL_ISSUE", "STAFF_REVIEW", "LATE"].includes(record.waitingState));
  const counts = {
    ready: windowRecords.filter((record) => record.waitingState === "READY_TO_START").length,
    waiting: windowRecords.filter((record) => ["VISITOR_WAITING", "PRISONER_WAITING", "BOTH_PRESENT"].includes(record.waitingState)).length,
    attention: windowRecords.filter((record) => ["TECHNICAL_ISSUE", "STAFF_REVIEW", "LATE"].includes(record.waitingState)).length,
    upcoming: windowRecords.filter((record) => record.waitingState === "NOT_ARRIVED").length,
  };
...
function IncidentsPage({ onNotify }: { onNotify: (message: string, tone?: Notice["tone"]) => void }) {
  ...
  const [createForm, setCreateForm] = useState({ title: "", description: "", incidentType: "OPERATIONAL", severity: "MEDIUM" });
  ...
  async function createIncident(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/control/incidents", {
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json", "Idempotency-Key": `incident-create:${crypto.randomUUID()}` },
        credentials: "include",
        body: JSON.stringify(createForm),
      });
      const body = await response.json() as { error?: string; incidentId?: string };
      if (!response.ok) throw new Error(body.error || "Incident could not be created.");
      setCreateForm({ title: "", description: "", incidentType: "OPERATIONAL", severity: "MEDIUM" });
      setShowCreate(false);
      await refresh();
      if (body.incidentId) setSelectedId(body.incidentId);
      onNotify("Incident created and added to the facility register.", "success");
    } catch (error) { onNotify(error instanceof Error ? error.message : "Incident could not be created.", "error"); }
    finally { setCreating(false); }
  }
  return <><PageHeader eyebrow="Operations · Case management" title="Incidents" description="Persisted operational cases, append-only event history, and controlled resolution actions." actions={<><Status tone="red">{incidents.filter((incident) => !["RESOLVED", "CLOSED"].includes(incident.status)).length} OPEN</Status><Button variant="primary" onClick={() => setShowCreate(true)}>+ Report incident</Button></>} />{loading ? <div className="sv3-empty"><span>◌</span><strong>Loading incident register</strong><p>Reading facility-scoped operational cases.</p></div> : !incidents.length ? <EmptyState title="No incidents recorded" body="Operational problems will appear here when staff create a persisted incident case." /> : <div className="sv3-incident-layout"><section className="sv3-incident-list"><div className="sv3-incident-list-head"><span>OPEN INCIDENTS</span><button onClick={() => refresh().catch(() => undefined)}>Refresh</button></div>{incidents.map((incident) => <button className={`sv3-incident-row ${selected?.id === incident.id ? "active" : ""}`} key={incident.id} onClick={() => setSelectedId(incident.id)}><span className={`sv3-severity severity-${incident.severity.toLowerCase()}`}>{incident.severity}</span><strong>{incident.title}</strong><small>{incident.id} · {new Date(incident.created_at).toLocaleString()}</small><span className="sv3-incident-meta">{incident.appointment_id || incident.resource_id || "Facility-wide"} · {incident.status}</span><b>›</b></button>)}</section>{selected ? <article className="sv3-incident-detail"><div className="sv3-incident-detail-head"><div><span className={`sv3-severity severity-${selected.severity.toLowerCase()}`}>{selected.severity}</span><span className="sv3-eyebrow">INCIDENT {selected.id}</span><h2>{selected.title}</h2><p>{selected.description}</p></div><Status tone={selected.status === "OPEN" ? "red" : selected.status === "RESOLVED" || selected.status === "CLOSED" ? "green" : "orange"}>{selected.status}</Status></div><div className="sv3-incident-columns"><div><SectionLabel>Case facts</SectionLabel><dl><div><dt>Type</dt><dd>{selected.incident_type}</dd></div><div><dt>Reporter</dt><dd>{selected.reporter_name || "Staff"}</dd></div><div><dt>Assignee</dt><dd>{selected.assignee_name || "Unassigned"}</dd></div><div><dt>Related visit</dt><dd>{selected.appointment_id || "None"}</dd></div></dl></div><div><SectionLabel>Resolution</SectionLabel><p>{selected.resolution || "No resolution recorded."}</p><div className="sv3-header-actions"><Button onClick={() => command("acknowledge", "Monitoring officer acknowledged this incident for investigation.")} disabled={selected.status !== "OPEN"}>Acknowledge</Button><Button onClick={() => command("add_note", "Staff reviewed the incident and added an operational note.")}>Add note</Button><Button variant="primary" onClick={() => command("resolve", "Incident investigation completed; operational controls restored.")} disabled={["RESOLVED", "CLOSED"].includes(selected.status)}>Resolve</Button><Button variant="quiet" onClick={() => command("close", "Supervisor confirmed the incident record is complete.")} disabled={selected.status !== "RESOLVED"}>Close</Button></div></div></div></article> : null}</div>}{showCreate ? <div className="sv3-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !creating) setShowCreate(false); }}><form className="sv3-dialog" role="dialog" aria-modal="true" aria-labelledby="incident-create-title" onSubmit={createIncident}><div className="sv3-dialog-head"><div><span className="sv3-eyebrow">OPERATIONS · NEW CASE</span><h2 id="incident-create-title">Report an incident</h2><p>Create a facility-scoped case. The record, audit event, and notification outbox entry are committed together.</p></div><button type="button" aria-label="Close incident form" onClick={() => setShowCreate(false)} disabled={creating}>×</button></div><label className="sv3-dialog-field">Title<input required minLength={4} maxLength={160} value={createForm.title} onChange={(event) => setCreateForm({ ...createForm, title: event.target.value })} placeholder="e.g. Kiosk camera unavailable" /></label><label className="sv3-dialog-field">Incident type<select value={createForm.incidentType} onChange={(event) => setCreateForm({ ...createForm, incidentType: event.target.value })}><option value="OPERATIONAL">Operational</option><option value="TECHNICAL">Technical</option><option value="SAFETY">Safety</option><option value="SECURITY">Security</option><option value="VISITOR">Visitor support</option></select></label><label className="sv3-dialog-field">Severity<select value={createForm.severity} onChange={(event) => setCreateForm({ ...createForm, severity: event.target.value })}><option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option><option value="CRITICAL">Critical</option></select></label><label className="sv3-dialog-field">Description<textarea required minLength={8} maxLength={2000} value={createForm.description} onChange={(event) => setCreateForm({ ...createForm, description: event.target.value })} placeholder="Describe what happened, who is affected, and what staff observed." /></label><div className="sv3-dialog-actions"><Button type="button" variant="quiet" onClick={() => setShowCreate(false)} disabled={creating}>Cancel</Button><Button type="submit" variant="primary" disabled={creating}>{creating ? "Creating…" : "Create incident"}</Button></div></form></div> : null}</>;
}
...
function ComplianceTab({ tab, onNotify }: { tab: string; onNotify: (message: string, tone?: Notice["tone"]) => void }) {
  ...
  useEffect(() => {
    ...
    const securityResetTimer = tab === "Security Events" ? window.setTimeout(() => { if (active) { setSecurityEvents([]); setSecurityError(false); } }, 0) : null;
    if (tab === "Security Events") requests.push(fetch("/api/control/security-events", { cache: "no-store" }).then(async (response) => { if (!response.ok) throw new Error("SECURITY_EVENTS_UNAVAILABLE"); const body = await response.json() as { events?: ComplianceSecurityEvent[] }; if (active) setSecurityEvents(body.events || []); }).catch(() => { if (active) setSecurityError(true); }));
    const reportResetTimer = tab === "Reports" ? window.setTimeout(() => { if (active) { setAuditCount(null); setFinanceSummary(null); setReportError(false); } }, 0) : null;
    if (tab === "Reports") { requests.push(fetch("/api/control/reports", { cache: "no-store", credentials: "include", headers: { accept: "application/json" } }).then(async (response) => { if (!response.ok) throw new Error("REPORT_DATA_UNAVAILABLE"); const body = await response.json() as { audit?: { eventCount?: number }; finance?: typeof financeSummary }; if (active) { setAuditCount(body.audit?.eventCount ?? null); setFinanceSummary(body.finance || null); } }).catch(() => { if (active) setReportError(true); })); }
...
type NotificationDeliveryAttempt = { id: string; notification_id: string; channel: string; attempt_number: number; status: string; error_message: string | null; started_at: string; finished_at: string | null };
type OutboxFailure = { id: string; event_type: string; aggregate_type: string; aggregate_id: string | null; payload: string; correlation_id: string; status: string; attempt_count: number; available_at: string; last_error: string | null; created_at: string; deliveryAttempts?: NotificationDeliveryAttempt[] };

function notificationAttemptSummary(event: OutboxFailure) {
  const attempts = event.deliveryAttempts || [];
  if (!attempts.length) return "No channel attempt records";
  const latestByChannel = new Map<string, NotificationDeliveryAttempt>();
  for (const attempt of attempts) {
    const current = latestByChannel.get(attempt.channel);
    if (!current || attempt.attempt_number > current.attempt_number) latestByChannel.set(attempt.channel, attempt);
  }
  return [...latestByChannel.values()].map((attempt) => `${attempt.channel}: ${attempt.status.replaceAll("_", " ").toLowerCase()}`).join(" · ");
}
